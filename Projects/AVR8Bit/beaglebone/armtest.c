...
int main(){
	...
	int status;
	int fd[2];
	struct board boards[7] = {{0x10, "Base rotation", 0, 128, -1}, {0x11, "Shoulder", 0, 192, -1}, {0x12, "Elbow", 1, 128, -1}, {0x13, "Forearm rotation", 0, 64, -1}, {0x14, "Differential wrist 1", 0, 128, -1}, {0x15, "Differential wrist 2", 0, 128, -1}, {0x16, "Hand", 0, 128, -1}};
	cur = boards;
	direction = cur->dir;
	initscr();
	noecho();
	...
	while((ch = getch()) != 'q'){
		...
		if(cur->mode == 1){
			mvprintw(3, 0, "Target position: ");
			mvprintw(4, 0, "Target velocity: ");
		}
