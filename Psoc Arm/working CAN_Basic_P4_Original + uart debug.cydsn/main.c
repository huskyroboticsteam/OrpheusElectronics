...
int main()
{
    ...
    CAN_GlobalIntEnable();
    //CAN_Start();
    sprintf(txData, "Can Init:  %u\r\n", CAN_Init());
    UART_UartPutString(txData);
    CAN_GlobalIntEnable();
   /* int test = CAN_Enable();
    while(test != CYRET_SUCCESS) {
        test = CAN_Start();   
    }*/
    sprintf(txData, "++++++++++++++++++++++++++++++++++++Can Enable:  %u\r\n", CAN_Enable());
    UART_UartPutString(txData);
    ...
    for(;;)
    {
        sprintf(txData, "Can error state:  %u\r\n", CAN_GetErrorState());
        UART_UartPutString(txData);
        sprintf(txData, "Can error msg:  %lx\n", CAN_ReadErrorCaptureRegister());
        UART_UartPutString(txData);
        if (ADC_IsEndConversion(ADC_RETURN_STATUS))
        {
            ...
            CAN_TX_DATA_BYTE1(CAN_TX_MAILBOX_ADCdata, 1);
            CAN_TX_DATA_BYTE2(CAN_TX_MAILBOX_ADCdata, 2);
            CAN_TX_DATA_BYTE3(CAN_TX_MAILBOX_ADCdata, 3);
            CAN_TX_DATA_BYTE4(CAN_TX_MAILBOX_ADCdata, 4);
            CAN_TX_DATA_BYTE5(CAN_TX_MAILBOX_ADCdata, 5);
            ...
            UART_UartPutString(txData);
            sprintf(txData, "ADC out: %x + %x = %x\r\n", HI8(resMilliVolts),LO8(resMilliVolts), resMilliVolts);
            UART_UartPutString(txData);
        }
        ...
        if (switchState != ReadSwSwitch())    /* Switch state changed status */
        {
            ...
            /* Send CAN message with switch state */
                sprintf(txData, "Can send switch msg:  %u\r\n", CAN_SendMsgswitchStatus());
                UART_UartPutString(txData);
        }
