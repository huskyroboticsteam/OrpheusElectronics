...
#include "Switch1.h"
    ...
    #define Switch1_SetP4PinDriveMode(shift, mode)  \
    do { \
        Switch1_PC =   (Switch1_PC & \
                                (uint32)(~(uint32)(Switch1_DRIVE_MODE_IND_MASK << \
                                (Switch1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Switch1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Switch1_SetP4PinDriveMode(shift, mode)  \
        do { \
            Switch1_USBIO_CTRL_REG = (Switch1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Switch1_DRIVE_MODE_IND_MASK << \
                                    (Switch1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Switch1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Switch1_SetP4PinDriveMode(Switch1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Switch1_DR & (uint8)(~Switch1_MASK));
    drVal = (drVal | ((uint8)(value << Switch1_SHIFT) & Switch1_MASK));
    Switch1_DR = (uint32)drVal;
}
...
{
    return (uint8)((Switch1_PS & Switch1_MASK) >> Switch1_SHIFT);
}
...
{
    return (uint8)((Switch1_DR & Switch1_MASK) >> Switch1_SHIFT);
}
...
void Switch1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Switch1_INTCFG & (uint32)(~(uint32)position);
    Switch1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Switch1_INTSTAT & Switch1_MASK);
	Switch1_INTSTAT = maskedStatus;
    return maskedStatus >> Switch1_SHIFT;
}
